...
import { createAsyncPipeline, MaybeAsync, useContainer, Container, AsyncPipeline } from 'farrow-pipeline';
import {
  createSchemaValidator,
  ValidationError,
} from 'farrow-schema/validator';

import { ApiDefinition, TypeOfTypeable, ApiSchema, Typeable } from 'farrow-api'
...
export type Func<T extends ApiDefinition> = (input: TypeOfTypeable<T['input']>) => MaybeAsync<TypeOf<ToSchemaCtor<TypeableContentType<T['output']>>>>
...
) => MaybeAsync<Prettier<WarpeOutput<TypeOf<ToSchemaCtor<TypeableContentType<T['output']>>>>>>

export type FuncPipeline<T extends ApiDefinition = ApiDefinition> = AsyncPipeline<
...
export type FuncType<T extends ApiDefinition = ApiDefinition> = ApiImpl<T> &
  ApiSchema<T> &
  FuncPipeline<T> &
  FuncMethods<T>

export const createFunc = <T extends ApiDefinition>(definition: T, func: Func<T>): FuncType<T> => {
  ...
  const apiPipeline = createAsyncPipeline<TypeOfTypeable<T['input']>, Prettier<WarpeOutput<TypeOf<ToSchemaCtor<TypeableContentType<T['output']>>>>>>()
...
export const isFunc = <T extends ApiDefinition = ApiDefinition>(input: any): input is FuncType<T> => {
  return typeof input === 'function' && input?.type === 'Func'
}